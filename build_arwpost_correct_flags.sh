#!/bin/bash
# Build ARWpost with Correct Compilation Flags
# This script uses the proper FFLAGS and linking for ARWpost

set -e  # Exit on any error

echo "=== ARWpost Build with Correct Intel Fortran Flags ==="
echo "Building ARWpost with proper Intel Fortran flags and linking..."
echo ""
...
# Set CORRECT Intel Fortran compilation flags
FCFLAGS="-O2 -xHost -I${NETCDF}/include -extend-source -warn all"
LDFLAGS="-L${NETCDF}/lib"
LIBS="-lnetcdff -lnetcdf"

echo "CORRECT Intel Fortran compilation flags:"
...
# Try to use the configure script first
echo "Trying to use configure script..."
if [ -f "configure" ]; then
    echo "Using ARWpost configure script..."
    
    # Run configure with Intel compiler selection (option 2)
    echo "2" | ./configure
...
if [ -f "ARWpost" ]; then
    ...
    # Manual compilation with CORRECT Intel Fortran flags
    echo "Manual compilation with CORRECT Intel Fortran flags..."
    
    # Compile all available source files with CORRECT flags
    echo "Compiling all available source files with CORRECT flags..."
    ...
    # Compile each source file with CORRECT Intel Fortran flags
    ...
    # Link the executable with CORRECT libraries
    echo "Linking ARWpost executable with CORRECT libraries..."
