#!/bin/bash
# Full ARWpost Build Script - Incremental Approach
# This script builds the complete ARWpost by starting with working version and adding modules

set -e  # Exit on any error

echo "=== Full ARWpost Build Script - Incremental Approach ==="
echo "Building complete ARWpost by starting with working version and adding modules..."
echo ""
...
WORKING_DIR="/home/apps/chpc/earth/ARWpost"
...
echo "Working ARWpost directory: ${WORKING_DIR}"
echo ""

# Check if working ARWpost exists
if [ ! -d "${WORKING_DIR}" ]; then
    echo "✗ Working ARWpost not found at: ${WORKING_DIR}"
    echo "Please ensure the working ARWpost is installed first"
    exit 1
fi

echo "✓ Found working ARWpost at: ${WORKING_DIR}"
...
# Copy working ARWpost as base
echo "Copying working ARWpost as base..."
cp -r "${WORKING_DIR}"/* "${BUILD_DIR}/"
cd "${BUILD_DIR}"

# Copy source files for additional modules
echo "Copying source files for additional modules..."
cp -r "${SOURCE_DIR}/ARWpost/src"/* "${BUILD_DIR}/src/"
